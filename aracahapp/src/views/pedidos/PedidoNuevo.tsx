// src/views/pedidos/PedidoNuevo.tsx
'use client';
...
type Linea = {
  id_producto: number | '';
  cantidad: number | '';
  precio_unitario?: number | ''; // opcional (solo visual si decides no enviarlo)

  // Solo visual (producto-lite)
  sku?: string;
  nombre?: string;
...
export default function PedidoNuevo() {
  ...
  useEffect(() => { document.title = 'Nuevo pedido | Sistema Aracah'; }, []);

  // Default +7 días
  useEffect(() => {
    if (!fechaCompromiso) setFechaCompromiso(dayjs().add(7, 'day').format('YYYY-MM-DD'));
  ...
  const onChangeRow = async (idx: number, field: keyof Linea, value: any) => {
    ...
    if (field === 'id_producto') {
      ...
      try {
        // Reutilizamos el auxiliar de cotizaciones
        ...
        setRows(prev => prev.map((row, i) => i === idx ? ({
          ...
          // Solo visual. Si no envías precio_unitario, el SP tomará catálogo.
  ...
  const impuestoCalc = rows.reduce((acc, r) => {
    const c = Number(r.cantidad || 0);
    const p = Number((r.precio_catalogo ?? r.precio_unitario) || 0);
    const t = Number(r.impuesto_tasa || 0) / 100;
    return acc + (c * p * t);
  }, 0);
  const totalCalc = subtotal + impuestoCalc;

  // ✅ Solo líneas válidas (imprescindible para que el BE no rechace)
  const lineasValidas = useMemo(() => {
    return rows
      .map(r => ({
        id_producto: Number(r.id_producto || 0),
  ...
  const canSubmit = !!token && !!idCliente && lineasValidas.length > 0;
  ...
  const onSubmit = async () => {
    try {
      ...
      const body = {
        ...
        detalle: lineasValidas, // ← sólo válidas
      };

      const resp = await fetch('http://localhost:3001/api/pedidos/manual', {
        method: 'POST',
        headers,
      ...
      const js = await resp.json(); // { id_pedido, numero? }
      setOkMsg('Pedido creado exitosamente.');
      setSnackOpen(true);
      setTimeout(() => navigate(`/pedidos/${js.id_pedido}`), 900);
    } catch (e: any) {
      setErr(e.message || 'Error al crear el pedido');
    }
  };

  return (
    <PageContainer title="Nuevo pedido | Sistema Aracah" description="Crear un pedido manual">
      <Box>
        <Stack direction="row" alignItems="center" spacing={2} mb={2}>
          <Typography variant="h4" fontWeight={700}>Nuevo pedido</Typography>
          <Box flexGrow={1} />
        ...
        <Paper sx={{ p: 2, mb: 2 }}>
          ...
          <Stack direction="row" alignItems="center" justifyContent="space-between" mb={1}>
            ...
            <Button onClick={onAddRow}>Agregar línea</Button>
          </Stack>
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell width={120} align="right">Cantidad</TableCell>
                <TableCell width={160} align="right">Precio (cat.)</TableCell>
                <TableCell width={140} align="right">Impuesto</TableCell>
                <TableCell />
              </TableRow>
            </TableHead>
            <TableBody>
              {rows.map((r, idx) => (
                <TableRow key={idx}>
                  ...
                  <TableCell align="right">
                    <Button color="error" onClick={() => onDelRow(idx)} disabled={rows.length === 1}>Eliminar</Button>
                  </TableCell>
                </TableRow>
              ))}
              {rows.length === 0 && (
          ...
          <Stack direction="row" mt={2} alignItems="center" justifyContent="flex-end" spacing={3}>
            <Typography variant="subtitle1">Subtotal: {fmt(subtotal)}</Typography>
            <Typography variant="subtitle1">Impuesto: {fmt(impuestoCalc)}</Typography>
            <Typography variant="h6">Total: {fmt(totalCalc)}</Typography>
          </Stack>
